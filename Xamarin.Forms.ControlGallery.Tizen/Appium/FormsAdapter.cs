...
namespace Xamarin.Forms.ControlGallery.Tizen
{
    public class FormsAdapter : BaseAdapter
    {
        ...
        {
            Platform.Tizen.Forms.ViewInitialized += (s, e) =>
            {
                if (e.View is Page p)
                {
                    p.Appearing += (ss, ee) =>
                ...
                {
                    if ((ee.PropertyName == "Renderer") && (TPlatform.GetRenderer((BindableObject)ss) == null))
                    {
        ...
        {
            var nativeView = (ElmSharp.GenList)TPlatform.GetOrCreateRenderer(list).NativeView;
